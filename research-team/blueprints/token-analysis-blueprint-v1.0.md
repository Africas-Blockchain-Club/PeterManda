...
- *Crucially, all math and formulas MUST be formatted in plain text standard markdown (e.g. `100 / 50 = 2`). The use of LaTeX math syntax (such as `\frac{}` or `\\[ \\]`) is strictly forbidden to ensure clean PDF rendering.*
...
**Goal**: Understand what the token actually does and its current mindshare on social platforms (especially X/Twitter).

### 0.1 High-Level Overview
- What is the token's primary utility and narrative? 
- Which sector does it belong to (e.g., L1, AI, Memecoin, DeFi)?

### 0.2 Social Sentiment & Mindshare
- **Platform Focus**: X (formerly Twitter).
- Are key opinion leaders (KOLs) or major accounts talking about it?
- Is the sentiment overwhelmingly euphoric (warning sign) or quietly accumulating? 
- Is there a specific catalyst driving current conversations (e.g., a mainnet launch, airdrop, or partnership)?
...
### 1.5 Protocol Fundamentals (The Value Layer)
**Action**: Evaluate the raw fundamental health of the token if it functions as a DeFi protocol.

- **Total Value Locked (TVL)**: Compare the token's TVL (from DefiLlama) against its Market Cap and Fully Diluted Valuation (FDV).
- **FDV/TVL Ratio**: 
  - A ratio < 1.0 often indicates a deeply undervalued, cash-rich protocol.
  - A ratio > 5.0 indicates the token is trading on future speculative premium rather than current captured utility.
  - *If TVL data is unavailable but the token claims to be DeFi*, heavily penalize the Blueprint Score as a high-risk speculative asset or potential vapourware.
...
- **Entry Trigger**: e.g. "Price stabilisation above $1.00 with positive OI flip"
- **Exit Trigger**: e.g. "Kill switch activation (thin order books or supply influx)"

### 4.4 AI Model Comparison (Added by Pipeline)
- **Action**: The orchestration pipeline runs this blueprint asynchronously across multiple AI models (e.g., Llama 3, Gemini, Qwen).
- **Output**: The pipeline automatically appends a markdown table at the very end of this document comparing the Blueprint Scores and verdicts generated by each competitor model, declaring the highest score the "Winning Model."*
...
- **Primary Source of Truth**: DexScreener
- **Core (Secondary Checks)**: CoinGecko, CoinMarketCap, Arkham, Glassnode
