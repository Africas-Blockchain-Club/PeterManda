...
"""
    ...
    st.info("Reports are generated by an Anthropic model (Sonnet by default). Select Opus for maximum analytical depth. Reports are table-first for quick at-a-glance analysis.")

    col1, col2, col3 = st.columns([2, 1, 1])
    with col1:
        tokens = st.multiselect("Select Tokens", ["BTC", "ETH"], default=["BTC", "ETH"])
    with col2:
        custom_token = st.text_input("Or enter a custom token", placeholder="e.g., ENA, SOL")
    with col3:
