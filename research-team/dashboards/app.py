...
"""
    ...
    st.info("All 3 AI models (Gemini, Llama 4 Maverick, Qwen 3) will generate reports. The one with the **highest Blueprint Score** is saved. Reports always overwrite the previous version to stay current.")
        ...
        if not all_tokens:
            ...
            for token in all_tokens:
                with st.status(f"Generating {token.upper()} report...", expanded=True) as status:
                    try:
                        path = generate_report(token, log_fn=status.write)
    ...
    if st.session_state.get("last_generated_tokens"):
        ...
        st.caption("Synthesised from the forensic report in plain English. Haiku is fast and cheap; Sonnet goes deeper.")
    ...
    if matching_rel_path:
        ...
        with st.container(border=True):
            brief_col, btn_col = st.columns([3, 1])
            with brief_col:
                st.markdown("#### AI Investment Brief")
                st.caption(
                    "Plain-English synthesis in Peter's voice. "
